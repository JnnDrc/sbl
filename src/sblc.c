...
int main(int argc, char* argv[]){
    ...
    while(fgets(line,sizeof(line),fin)){
        ...
        if      (streq(op,"add")) ilist_add(&il,MAKE_OP(OP_ADD));
            ...
            fprintf(stderr,"[ERROR]: unknown operation %s, at %zu", op,l);
            return -3;
        }
    }
    ...
    // header
    fwrite("SB",2,1,fout);                              // magic number
    fwrite(&il.size,sizeof(uint16_t),1,fout);           // amount of instructions
    uint32_t ctoff = il.size * sizeof(*il.data);        // const table offset
    fwrite(&ctoff,sizeof(ctoff),1,fout);
    // instructions
    fwrite(il.data,sizeof(*il.data),il.size,fout);
    // const table
    fwrite(ct.data,sizeof(*ct.data),ct.size,fout);
